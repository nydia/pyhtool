...
from pyhtool.gui.Const import _const
import functools
from . import Mouse

if sys.platform.startswith("java"):
    raise NotImplementedError("Jython is not yet supported by pyhtool.")
    ...
    raise NotImplementedError("Your platform (%s) is not supported by pyhtool." % (platform.system()))
...
def failSafeCheck():
    if FAILSAFE and tuple(Mouse.position()) in FAILSAFE_POINTS:
        raise Exception(
            "pyhtool fail-safe triggered from mouse moving to a corner of the screen. To disable this fail-safe, set pyhtool.FAILSAFE to False. DISABLING FAIL-SAFE IS NOT RECOMMENDED."
        )